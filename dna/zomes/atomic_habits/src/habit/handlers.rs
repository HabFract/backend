...
pub fn get_habit(entry_hash: EntryHashB64) -> ExternResult<Option<Habit>> {
  ...
  match maybe_element {
    None => Ok(None),
    Some(element) => {
      let habit: Habit = element.entry()
        .to_app_option()?
        .ok_or(WasmError::Guest("Could not deserialize element to Habit.".into()))?;
...
#[hdk_extern]
pub fn create_habit(habit: Habit) -> ExternResult<NewHabitOutput> {
  let header_hash = create_entry(&habit)?;
...
pub fn update_habit(input: UpdateHabitInput) -> ExternResult<NewHabitOutput> {
  let header_hash = update_entry(HeaderHash::from(input.original_header_hash), &input.updated_habit)?;
